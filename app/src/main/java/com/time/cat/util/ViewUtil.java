...
public class ViewUtil {
    ...
    public static float px2dp(float px) {
        return px / TimeCatApp.getInstance().getResources().getDisplayMetrics().density;
    }

    public static float sp2px(float sp) {
        return TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, sp, TimeCatApp.getInstance().getResources().getDisplayMetrics());
    }
...
//	    if (myView.getParent() == myView.getRootView())
    ...
    public static void requestInputMethodIfShow(final EditText view) {
        final InputMethodManager imm = (InputMethodManager) TimeCatApp.getInstance().getSystemService(Context.INPUT_METHOD_SERVICE);
        if (imm.isActive()) {
            view.requestFocus();
